...
def generate_world():
    ...
    # Add small fishes
    for i in range(math.ceil(BASE_PLANKTON * 0.40)):
        small_fish = SmallFish()
        all_sprites.add(small_fish)
        total_small_fish += 1

    # Add big fishes
    for i in range(math.ceil(BASE_PLANKTON * 0.02)):
        big_fish = BigFish()
    ...
    # Add whales
    for i in range(math.ceil(BASE_PLANKTON * 0.002)):
        whale = Whale()
    ...
    print("-------")
    print("Total non-plankton generated: ", total_generated)
